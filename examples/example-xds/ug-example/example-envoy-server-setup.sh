...
# create namespaces
kubectl apply -f ug-example/gke-namespace-create.yaml

kubectl label namespace example-grpc-client --overwrite istio-injection-
kubectl label namespace example-grpc-server --overwrite istio-injection=enabled

# first deploy our service
cat ug-example/gke-server-envoy-deployment.yaml | envsubst | kubectl apply -f -
...
if [ x${NEG_NAME} != xexample-grpc-server ]; then
    echo NEG_NAME is not set, some issue with the deployment ug-example/gke-server-envoy-deployment.yaml. Exiting...
    exit 1
fi
...
gcloud beta network-services endpoint-policies import ep_mtls_envoy \
  --source=ug-example/ep-mtls-envoy.yaml --location=global
...
gcloud compute target-http-proxies create example-grpc-proxy --url-map example-grpc-url-map
...
gcloud compute forwarding-rules create example-grpc-forwarding-rule --global \
  ...
  --target-http-proxy=example-grpc-proxy --ports 8000 \
  --network default
