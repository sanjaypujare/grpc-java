...
gcloud beta network-services endpoint-policies delete ep_mtls_envoy --location=global -q
...
gcloud compute target-http-proxies delete example-grpc-proxy -q
...
kubectl delete -f ug-example/gke-server-envoy-deployment.yaml
kubectl delete -f ug-example/gke-namespace-create.yaml
