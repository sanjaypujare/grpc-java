...
gcloud compute health-checks create tcp example-test-health-check --use-serving-port
...
gcloud compute backend-services create example-grpc-service --global \
    --health-checks example-test-health-check   --load-balancing-scheme INTERNAL_SELF_MANAGED --protocol HTTP2
