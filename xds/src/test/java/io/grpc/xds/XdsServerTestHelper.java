...
class XdsServerTestHelper {
  ...
  static XdsClient.LdsResourceWatcher startAndGetWatcher(
          ...
          XdsClient mockXdsClient, int port) {
    xdsClientWrapperForServerSds.start(mockXdsClient, "grpc/server?addr=%s");
    ArgumentCaptor<XdsClient.LdsResourceWatcher> listenerWatcherCaptor = ArgumentCaptor.forClass(null);
    verify(mockXdsClient).watchLdsResource(eq("grpc/server?addr=0.0.0.0:" + port), listenerWatcherCaptor.capture());
