...
final class SdsSslContextProvider<K> extends SslContextProvider<K> implements SdsClient.SecretWatcher {
    ...
    SdsSslContextProvider(
        ...
        if (certSdsConfig != null && certSdsConfig.isInitialized()) {
            certSdsClient = SdsClient.Factory.createWithNettyChannel(certSdsConfig, node,
        ...
        if (validationContextSdsConfig != null && validationContextSdsConfig.isInitialized()) {
            validationContextSdsClient = SdsClient.Factory.createWithNettyChannel(validationContextSdsConfig, node,
                Executors.newSingleThreadExecutor(), null);
    ...
    void performCallback(final SslContextGetter sslContextGetter, final Callback callback, Executor executor) {
        executor.execute(
            new Runnable() {
                @Override
                public void run() {
